"""Focused regression tests for Level 2 parallax normalization."""
...
SPEC = importlib.util.spec_from_file_location("level2_parallax", ROOT / "tools/asset_pipeline/process_level2_parallax.py")
assert SPEC and SPEC.loader
MODULE = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(MODULE)


class Level2ParallaxProcessingTests(unittest.TestCase):
    ...
    def test_moving_plate_fits_without_nonuniform_scaling(self) -> None:
        ...
        output = MODULE.remove_boundary_connected_magenta(MODULE.fit_moving_plate(source))
        self.assertEqual((1320, 540), output.size)
